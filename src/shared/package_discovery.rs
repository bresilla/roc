...
fn is_hidden_package_relative(package: &Package, base_path: &Path) -> bool {
    // Consider a package hidden if its name starts with underscore or if it's
    // contained in a hidden directory *relative to the discovery base path*.
    //
    // We must not treat a package as hidden just because its absolute path
    // happens to contain dot-prefixed directories (e.g. `tempfile` creates
    // temporary directories like `/tmp/.tmpXXXX`).
    if package.name.starts_with('_') {
        return true;
    }

    let rel = match package.path.strip_prefix(base_path) {
        Ok(p) => p,
        Err(_) => package.path.as_path(),
    };

    rel.iter()
        .any(|component| component.to_str().map_or(false, |s| s.starts_with('.')))
}
...
fn is_source_package(package: &Package) -> bool {
    ...
    if path_str.contains("/install/")
        || path_str.contains("\\install\\")
...
fn parse_package_xml(package_xml_path: &Path) -> Result<Package> {
    ...
    let build_type = root
        ...
        .and_then(|export| export.descendants().find(|n| n.has_tag_name("build_type")))
        .and_then(|n| n.text())
        .map(BuildType::from)
