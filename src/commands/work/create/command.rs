...
fn create_package(matches: ArgMatches) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let package_format = matches.get_one::<String>("package_format")
        .map(|s| s.as_str())
        .unwrap_or("3");
    let default_description = format!("TODO: Package description for {}", package_name);
    let description = matches.get_one::<String>("description")
        .map(|s| s.as_str())
        .unwrap_or(&default_description);
    let license = matches.get_one::<String>("license")
        .map(|s| s.as_str())
        .unwrap_or("TODO: License declaration");
    let destination_directory = matches.get_one::<String>("destination_directory")
        .map(|s| s.as_str())
        .unwrap_or(".");
    let build_type = matches.get_one::<String>("build_type")
        .map(|s| s.as_str())
        .unwrap_or("ament_cmake");
    let dependencies: Vec<&str> = matches.get_many::<String>("dependencies")
        .map(|vals| vals.map(|s| s.as_str()).collect())
        .unwrap_or_default();
    let maintainer_email = matches.get_one::<String>("maintainer_email")
        .map(|s| s.as_str())
        .unwrap_or("todo@example.com");
    let maintainer_name = matches.get_one::<String>("maintainer_name")
        .map(|s| s.as_str())
        .unwrap_or("TODO");
    ...
    if package_path.exists() {
        return Err(format!("Package directory '{}' already exists", package_path.display()).into());
    }
    ...
    match build_type {
        ...
        "ament_python" => {
            create_python_package(&package_path, package_name, node_name)?;
        }
    ...
    if build_type == "ament_cmake" {
        println!("   🔧 Build with: roc work build --packages-select {}", package_name);
...
fn create_cmake_package(
    ...
    if let Some(library_name_str) = library_name {
        ...
        println!("  📝 Created C++ library: include/{}/{}.hpp, src/{}.cpp", 
...
fn create_python_package(
    ...
    if let Some(node_name_str) = node_name {
        ...
        fs::write(&node_path, node_content)?;
        println!("  📝 Created Python node: {}/{}.py", package_name, node_name_str);
    }
